...
typedef struct MediaBufPool_s
{
    ...
    int32_t              vi_mem_pool_region_id; /**< Alloc from which mem pool region */
} MediaBufPool_t;
...
typedef struct MediaBufPoolMemory_s
{
    ...
    uint32_t    pBufferMemorySramCb; /**< Actual memory of Cb plane for Nano handshake */
    uint32_t    pBufferMemorySramCr; /**< Actual memory of Cr plane for Nano handshake */
#endif
...
#ifdef ISP_MI_HANDSHAKE_NANO
/**
 * @brief   Create a buffer pool by sram mem for Nano handshake.
 *
 * @param   pBufPool        Pointer to buffer pool object to be created.
 * @param   pPoolConfig     Pointer to config params for buffer pool.
 * @param   poolMemory      Pool memory information.
 ...
 * @return  Status of operation.
 *****************************************************************************/
extern ScmiReturnType MediaBufPoolSramCreate(MediaBufPool_t*       pBufPool,
                                  MediaBufPoolConfig_t* pPoolConfig,
                                  MediaBufPoolMemory_t  poolMemory);

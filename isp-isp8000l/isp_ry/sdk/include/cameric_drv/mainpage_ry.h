 ...
 * to register or release interrupt service routines (HalConnectIrq_ry / HalDisconnectIrq_ry).
 ...
 *
 * An instance is created by @ref CamerIcDriverInit_ry and released by @ref
 * CamerIcDriverRelease_ry. After a static configuration of the CamerIC driver
 * instance, which includes the configutration of the input acquistion (resoulution,
 * bayerpattern, cropping window, ... ) it can be started by calling @ref CamerIcDriverStart_ry.
 * If a statically reconfiguring or a releasing the driver instance necessary
 * the instance needs to be stopped with calling @ref CamerIcDriverStop_ry.
 *
 * The following figure presents the initialization flow chart of the CamerIC Driver.
 * First the CamEngine fills in the configuration structure @ref CamerIcDrvConfig_ry_s.
 * This includes the base address of the CamerIC mapped register table and a HAL handle.
 * Than the Camengine calls @ref CamerIcDriverInit_ry, which creates and initializes the
 * driver context.
 *
 ...
 *
 * On success the CamerIcDriverInit_ry returns the handle of the new created CamerIc driver
 * instance. This handle is returned in the configuration structure (see
 * DrvHandle in @ref CamerIcDrvConfig_ry_s).
 *
 ...
 * CamerIcDriverStart_ry. In the following flowchart an static configuration is exemplary

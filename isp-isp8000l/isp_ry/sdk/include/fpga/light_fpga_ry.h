...
*/
/******************************************************************************/
/*! Module    : Light FPGA driver
*
* Hierarchy :
*
* Purpose   : User-level driver for the Light FPGA board
*
...
/**
 *  @file light_fpga.h
 *  @brief User-level driver for the Light FPGA board
 */

#ifndef _LIGHT_FPGA_H
#define _LIGHT_FPGA_H

#include <hal/hal_common_ry.h>
...
// these mainly influence byte reordering stuff in LightFPGABoard_DMARead/Write functions
// use LightFPGA_RawDMARead/Write functions to avoid byte reordering & stuff
#define FPGA_DATA_ITEM_SIZE     8
#define FPGA_DATA_BLOCK_SIZE    8
#define FPGA_DATA_BLOCK_STRIDE  8

#define FPGA_DMA_ALIGNMENT       (4*1024)    /**< DMA helper buffer alignment (*must* be a power of two!); used by LightFPGABoard_DMARead/Write functions only */
#define FPGA_DMA_SIZE_ALIGNMENT  32          /**< DMA transfer size alignment (*must* be a power of two!); used by LightFPGABoard_DMARead/Write functions only */
...
/* PLL config type. */
typedef struct _LightFPGAPLLConfig {
    uint32_t Fin;
    uint16_t M;
    uint16_t N;
    uint16_t C[10];
} LightFPGAPLLConfig_t;

/* PLL type. */
typedef struct _LightFPGAPLL {
    uint32_t reg_offset;
...
extern const LightFPGAPLL_t LightFPGAPLL_1; //vdu
extern const LightFPGAPLL_t LightFPGAPLL_2; //msvd, cam1 etc
...
uint32_t LightFPGA_Open(HalHandle_t HalHandle);


/** LightFPGA_Reset()
  * @brief  Reset the FPGA board's ISP module
  */
void LightFPGA_Reset(HalHandle_t HalHandle);

/** LightFPGA_Close()
  * @brief  Uninitialize and close the FPGA board.
  * @retval FPGA_RES_OK     operation successful
  * @retval FPGA_RES_NODEV  FPGA board not initialized (need to call LightFPGA_Open() first) */
uint32_t LightFPGA_Close(HalHandle_t HalHandle);
...
uint32_t LightFPGA_ReadReg_Priv(int32_t isp_fd, uint32_t address);
...
uint32_t LightFPGA_WriteReg_Priv(int32_t isp_fd, uint32_t address, uint32_t data);
...
/** LightFPGA_ReadReg()
  ...
  * @return the value read from the register, or FPGA_READ_ERROR if any error occured */
INLINE uint32_t LightFPGA_ReadReg(int32_t isp_fd, uint32_t address) {
#if FPGA_USE_MMAP
    return fpga_regs_mmap[FPGA_REG_ADDRESS_MOD(address) >> 2];
#else
    return LightFPGA_ReadReg_Priv(isp_fd, FPGA_REG_ADDRESS_MOD(address));
#endif
}
...
/** LightFPGA_WriteReg()
  ...
  * @retval FPGA_RES_NODEV  FPGA board not initialized (need to call LightFPGA_Open() first)
  * @retval FPGA_RES_FAIL   could not write value */
INLINE uint32_t LightFPGA_WriteReg(int32_t isp_fd, uint32_t address, uint32_t data) {
#if FPGA_USE_MMAP
    fpga_regs_mmap[FPGA_REG_ADDRESS_MOD(address) >> 2] = data;
    return FPGA_RES_OK;
#else
    return LightFPGA_WriteReg_Priv(isp_fd, FPGA_REG_ADDRESS_MOD(address), data);
#endif
}
...
uint32_t LightFPGA_ReadMis(int32_t isp_fd, uint32_t* pAry);

/** LightFPGA_RawDMARead()
...
uint32_t LightFPGA_RawDMARead(HalHandle_t HalHandle, void* data, uint32_t address, uint32_t size);



/** LightFPGA_RawDMAWrite()
  ...
  * @retval FPGA_RES_FAIL   transfer failed
  * @retval FPGA_RES_NODMA  DMA not available on this board */
uint32_t LightFPGA_RawDMAWrite(HalHandle_t HalHandle, void* data, uint32_t address, uint32_t size);
...
/** LightFPGA_DMARead()
  ...
  * @retval FPGA_RES_FAIL   transfer failed
  * @retval FPGA_RES_NODMA  DMA not available on this board */
uint32_t LightFPGA_DMARead(HalHandle_t HalHandle, uint8_t* data, uint32_t address, uint32_t size);
...
/** LightFPGA_DMAWrite()
  ...
  * @retval FPGA_RES_FAIL   transfer failed
  * @retval FPGA_RES_NODMA  DMA not available on this board */
uint32_t LightFPGA_DMAWrite(HalHandle_t HalHandle, uint8_t* data, uint32_t address, uint32_t size);
...
uint32_t LightFPGA_SetupIRQ(fpga_irq_handle_t *irq, uint32_t mis_addr, uint32_t cis_addr, uint32_t timeout);
...
/** LightFPGA_StopIRQ()
  ...
  * @retval FPGA_RES_NODEV  FPGA board not initialized (need to call LightFPGA_Open() first)
  * @retval FPGA_RES_FAIL   interrupt setup failed */
uint32_t LightFPGA_StopIRQ(fpga_irq_handle_t *irq);


/** LightFPGA_WaitForIRQ()
...
uint32_t LightFPGA_WaitForIRQ(HalIrqCtx_t *pIrqCtx, uint32_t irq_src, uint32_t *irq_status);
...
uint32_t LightFPGA_CancelIRQ(fpga_irq_handle_t *irq);


/** LightFPGA_SetPLLConfig()
...
uint32_t LightFPGA_SetPLLConfig(const LightFPGAPLL_t* pll, const LightFPGAPLLConfig_t* cfg);


#if FPGA_USE_KERNEL_IRQ_HANDLING

#define LightFPGA_ForbidIRQPolling(x) do { } while (0)
#define LightFPGA_AllowIRQPolling(x)  do { } while (0)

#else

/** LightFPGA_ForbidIRQPolling()
  * @brief  Temporally forbid IRQ polling */
void LightFPGA_ForbidIRQPolling(fpga_irq_handle_t *irq);

/** LightFPGA_AllowIRQPolling()
  * @brief  Re-allow IRQ polling */
void LightFPGA_AllowIRQPolling(fpga_irq_handle_t *irq);

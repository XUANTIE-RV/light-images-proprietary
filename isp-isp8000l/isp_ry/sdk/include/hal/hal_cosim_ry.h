...
*   implementation.\n
*   Do not include directly! Include hal_api_ry.h instead.
*
* </pre>
*/
/*****************************************************************************/
...
#include <stdio.h>
//#include "i2c_drv/i2c_drv_ry.h"

//#include "altera_fpga_ry.h"
...
#include <mrv_all_regs_macro_defs_ry.h>


#include <mrv_all_regs_addr_map_ry.h>
#include <mrv2_all_regs_addr_map_ry.h>
#include <mrv_all_reg_description_ry.h>
#include <mrv_all_reg_descr_idx_ry.h>
  //#include <mrv_all_regs_mask_ry.h>
#include <mrv_all_regs_ry.h>

#include <vdu_all_regs_addr_map_ry.h>
#include <vdu_all_reg_description_ry.h>
#include <vdu_all_reg_descr_idx_ry.h>
#include <vdu_all_regs_mask_ry.h>

#include <fpga_sys_ctrl_regs_addr_map_ry.h>
#include <fpga_sys_ctrl_reg_description_ry.h>
#include <fpga_sys_ctrl_reg_descr_idx_ry.h>
#include <fpga_sys_ctrl_regs_mask_ry.h>

#include <iic1_regs_addr_map_ry.h>
#include <iic2_regs_addr_map_ry.h>
#include <iic3_regs_addr_map_ry.h>
#include <iic1_reg_description_ry.h>
#include <iic1_reg_descr_idx_ry.h>
#include <iic1_regs_mask_ry.h>
 ...
 * HalWriteSysReg_ry()
 *****************************************************************************/
INLINE void HalWriteSysReg_ry( HalHandle_t handle, uint32_t reg_address, uint32_t value )
{
    (void) handle;
    DCT_ASSERT(handle != NULL);
 ...
 * HalReadSysReg_ry()
 *****************************************************************************/
INLINE uint32_t HalReadSysReg_ry( HalHandle_t handle, uint32_t reg_address )
{
    (void) handle;
    DCT_ASSERT(handle != NULL);
    //    return *( (volatile uint32_t *) reg_address );
 ...
 * HalReadMaskedReg_ry()
 *****************************************************************************/
INLINE uint32_t HalReadMaskedReg_ry( HalHandle_t handle, uint32_t reg_address, uint32_t reg_mask, uint32_t shift_mask )
{
    (void) handle;
    DCT_ASSERT(handle != NULL);
 ...
 *****************************************************************************/
INLINE void HalWriteMaskedReg_ry( HalHandle_t handle, uint32_t reg_address, uint32_t reg_mask, uint32_t shift_mask, uint32_t value )
{
    (void) handle;
    DCT_ASSERT(handle != NULL);
...
{
    // just forward to HalClose_ry; it takes care of everything
    return HalClose_ry( HalHandle );
}
...
RESULT HalSetClock_ry( HalHandle_t HalHandle, uint32_t dev_mask, uint32_t frequency )
{
  ...
  if (dev_mask == HAL_DEVID_PCLK) {
    reset_val = HalReadSysReg_ry(HalHandle, MRV_FPGA_VEC_SYS_RESETN_REG);
    HalWriteSysReg_ry(HalHandle, MRV_FPGA_VEC_SYS_RESETN_REG, reset_val & 0xff7f);
    HalWriteSysReg_ry(HalHandle, MRV_FPGA_VEC_SYS_RESETN_REG, reset_val | 0x0080);
    ...
    HalWriteSysReg_ry(HalHandle, MRV_FPGA_RE_RECONFIG_REG, 0x1);
...
static uint32_t _set_single_pll_param(HalHandle_t HalHandle, const uint8_t counter_type, const uint8_t counter_param, const uint16_t value) {
  ...
  HalWriteSysReg_ry(HalHandle, MRV_FPGA_RE_COUNTER_TYPE_PARAM_REG, (counter_param << 4) | counter_type );
  HalWriteSysReg_ry(HalHandle, MRV_FPGA_RE_WR_RD_DATA_REG, value);
  HalWriteSysReg_ry(HalHandle, MRV_FPGA_RE_WRITE_PARAM_REG, 0x1);

 ...
 *****************************************************************************/
 #include "hal_common_ry.h"

#if defined( HAL_ALTERA )
  #include "hal_altera_pci_ry.h"
#elif defined( HAL_MOCKUP )
  #include "hal_mockup_ry.h"
#elif defined( HAL_COSIM )
  #include <hal/hal_cosim_ry.h>
#elif defined (HAL_CMODEL)
  #include "hal_cmodel_ry.h"
#elif defined (HAL_LIGHT_FPGA)
  #include "hal_light_fpga_ry.h"
#else
...
/**
 * @defgroup hal_api_ry Hardware Abstraction Layer interface
 * @{
 ...
 *****************************************************************************/
HalHandle_t HalOpen_ry( int ispID );


/*****************************************************************************/
/**
 * @brief   close the low level driver
 * @param   HalHandle   Handle to HAL session as returned by @ref HalOpen_ry.
 * @return  Result of operation.
 *
 * @note    Decrements internal ref count and closes low level driver if ref count reached zero.
 *****************************************************************************/
RESULT HalClose_ry( HalHandle_t HalHandle );
 ...
 * @note
 *****************************************************************************/
RESULT HalAddRef_ry( HalHandle_t HalHandle );
 ...
 *****************************************************************************/
RESULT HalDelRef_ry( HalHandle_t HalHandle );
 ...
 *          can't be correctly processed by \ref HalSetReset_ry() or @ref HalSetPower_ry()
 ...
 *                          camera data. NOTE: @ref HalSetClock_ry() must be called
 *                          to activate this new setting!
 * @return  Result of operation.
 *
 * @note    Device mask is bitwise OR (|) of HAL_DEVID_CAM_xxx.
 *****************************************************************************/
RESULT HalSetCamConfig_ry( HalHandle_t HalHandle, uint32_t dev_mask, bool_t power_lowact, bool_t reset_lowact, bool_t pclk_negedge );
 ...
 *****************************************************************************/
RESULT HalSetCamPhyConfig_ry( HalHandle_t HalHandle, uint32_t dev_mask, bool_t power_lowact, bool_t reset_lowact );


/*****************************************************************************/
/**
 * @brief   Enables/Disables reset of given devices. HAL takes care of polarity!
 *          See @ref HalSetCamConfig_ry() for details regarding CAM devices,
 *          and @ref HalSetCamPhyConfig_ry() for details regarding CAMPHY devices.
 * @param   HalHandle   Handle to HAL session as returned by @ref HalOpen_ry.
 ...
 *****************************************************************************/
RESULT HalSetReset_ry( HalHandle_t HalHandle, uint32_t dev_mask, bool_t activate );
 ...
 * @note    Device mask is bitwise OR (|) of HAL_DEVID_xxx.
 *****************************************************************************/
RESULT HalSetPower_ry( HalHandle_t HalHandle, uint32_t dev_mask, bool_t activate );
 ...
 * @note    Device mask is bitwise OR (|) of HAL_DEVID_CAM_xxx.
 *****************************************************************************/
RESULT HalSetClock_ry( HalHandle_t HalHandle, uint32_t dev_mask, uint32_t frequency );
 ...
 *****************************************************************************/
uint32_t HalAllocMemory_ry( HalHandle_t HalHandle, uint32_t byte_size );
 ...
 * @note    Chunks of n*4K byte with addresses aligned to 4K are used internally.
 *****************************************************************************/
RESULT HalFreeMemory_ry( HalHandle_t HalHandle, uint32_t mem_address );
 ...
 *****************************************************************************/
RESULT HalReadMemory_ry( HalHandle_t HalHandle, uint32_t mem_address, uint8_t* p_read_buffer, uint32_t byte_size );
 ...
 *          both addresses and transfer size exist!
 *****************************************************************************/
RESULT HalWriteMemory_ry( HalHandle_t HalHandle, uint32_t mem_address, uint8_t* p_write_buffer, uint32_t byte_size );
 ...
 *          both addresses and transfer size exist!
 *****************************************************************************/
RESULT HalMapMemory_ry( HalHandle_t HalHandle, uint32_t mem_address, uint32_t byte_size, HalMapMemType_t mapping_type, void **pp_mapped_buf );
 ...
 *          both addresses and transfer size exist!
 *****************************************************************************/
RESULT HalUnMapMemory_ry( HalHandle_t HalHandle, void* p_mapped_buf );
 ...
 * @note
 *****************************************************************************/
RESULT HalReadI2CMem_ry( HalHandle_t HalHandle, uint8_t bus_num, uint16_t slave_addr, uint32_t reg_address, uint8_t reg_addr_size, uint8_t *p_read_buffer, uint32_t byte_size );
 ...
 * @note
 *****************************************************************************/
RESULT HalWriteI2CMem_ry( HalHandle_t HalHandle, uint8_t bus_num, uint16_t slave_addr, uint32_t reg_address, uint8_t reg_addr_size, uint8_t *p_write_buffer, uint32_t byte_size );
 ...
 *****************************************************************************/
INLINE RESULT HalReadI2CReg_ry( HalHandle_t HalHandle, uint8_t bus_num, uint16_t slave_addr, uint32_t reg_address, uint8_t reg_addr_size, void *preg_value, uint8_t reg_size )
{
    ...
    return HalReadI2CMem_ry( HalHandle, bus_num, slave_addr, reg_address, reg_addr_size, (uint8_t*)preg_value, reg_size );
}
 ...
 *****************************************************************************/
INLINE RESULT HalWriteI2CReg_ry( HalHandle_t HalHandle, uint8_t bus_num, uint16_t slave_addr, uint32_t reg_address, uint8_t reg_addr_size, uint32_t reg_value, uint8_t reg_size )
{
    if ((reg_size < 1) || (reg_size > 4))
    {
        return RET_INVALID_PARM;
    ...
    return HalWriteI2CMem_ry( HalHandle, bus_num, slave_addr, reg_address, reg_addr_size, (uint8_t*)&reg_value, reg_size );
}
 ...
 *
 *  @param  Handle to HAL session as returned by @ref HalOpen_ry
 ...
 *
 *****************************************************************************/
RESULT HalEventGenerator_ry(    HalIrqCtx_t *pIrqCtx, HalIsrSrc_t isr_src, uint32_t misValue);
 ...
 *
 *****************************************************************************/
void HalFpgaDumpBuf_ry (char *buf, int len, char* str);
 ...
 *
 *****************************************************************************/
void HalFpgaReset_ry(HalHandle_t HalHandle);
#ifdef FPGA_SUPPORT_PACKAGE
void HalPhyRst_ry(HalHandle_t HalHandle);
void HalPhyCtrl_ry(HalHandle_t HalHandle);
#endif
/*****************************************************************************/
/**
 *          HalFpgaDereset_ry()
 ...
 *
 *****************************************************************************/
void HalFpgaDereset_ry(HalHandle_t HalHandle);
#ifndef FPGA_SUPPORT_PACKAGE
void HalMipiCsiinit_ry(HalHandle_t HalHandle);
void HalFpgaPreSet_ry(HalHandle_t HalHandle);
#endif
#ifdef FPGA_SUPPORT_PACKAGE
void HalFpgaPreSet_ry(HalHandle_t HalHandle);
void HalFpgaTpgCtrl_ry(HalHandle_t HalHandle);
void HalFpgaStreamControl_bus0_ry(HalHandle_t HalHandle, unsigned int stream_ctrl);
void HalFpgaStreamControl_bus1_ry(HalHandle_t HalHandle, unsigned int stream_ctrl);

int HalFpgaInitialRegisterFile_ry(HalHandle_t hHal,        const char *iniRegFileName);
int HalFpgaRegisterInitial_ry(HalHandle_t hHal, const char *registerFileName, int inibool);
#endif

...
/**
 * @file hal_light_fpga.h
 *
 * @brief   Light FPGA variant of Hardware Abstraction Layer
 *
 *          This header file exports the register IO interface realized as an
 *          Light FPGA board access inline function. You should use it for your
 *          PC implementation in combination with the Light FPGA board.
 * @note    Do not include directly! Include hal_api_ry.h instead with HAL_LIGHT defined.
 *
 *****************************************************************************/

#ifndef __HAL_LIGHT_FPGA_H__
#define __HAL_LIGHT_FPGA_H__
#include <ebase/trace_ry.h>
#include <fpga/light_fpga_ry.h>
#include <i2c_drv/i2c_drv_ry.h>

#include <cam_device/cam_device_buf_defs_ry.h>
 ...
 *          This function is used internally by HalReadMaskedReg_ry() but can also
 ...
 * @note    It is required to pass a variable (not a register!)
 *          and the masks (reg_mask and shift_mask) by using constant defines.
 *          This function is used internally by HalWriteMaskedReg_ry() but can also
...
INLINE uint32_t HalReadReg( HalHandle_t HalHandle, uint32_t reg_address )
{
    ...
    read = LightFPGA_ReadReg(HalHandleCtx->isp_fd, reg_address);
...
INLINE void HalWriteReg( HalHandle_t HalHandle, uint32_t reg_address, uint32_t value )
{
    ...
    (void) LightFPGA_WriteReg(HalHandleCtx->isp_fd, reg_address, value );
}
...
INLINE uint32_t HalReadSysReg_ry( HalHandle_t HalHandle, uint32_t reg_address )
{
   ...
   // DCT_ASSERT(HalHandle != NULL);
    ...
    return LightFPGA_ReadReg( pHalCtx->isp_fd, FPGA_REG_ADDRESS_MOD(reg_address) );
}
...
INLINE void HalWriteSysReg_ry( HalHandle_t HalHandle, uint32_t reg_address, uint32_t value )
{
    ...
    (void) LightFPGA_WriteReg( pHalCtx->isp_fd, FPGA_REG_ADDRESS_MOD(reg_address), value );
}

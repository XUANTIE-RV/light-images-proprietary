...
static bool mapRawBuffer(HalHandle_t hal, const PicBufMetaData_t *pSrcBuffer,
  ...
  *pDstBuffer = *pSrcBuffer;
  ...
  if (RET_SUCCESS != HalMapMemory_ry(hal, BaseAddr, PlaneSize, HAL_MAPMEM_READONLY,
...
static bool mapYCbCrBuffer(HalHandle_t hal, const PicBufMetaData_t *pSrcBuffer,
  ...
  *pDstBuffer = *pSrcBuffer;
  ...
  if (PIC_BUF_LAYOUT_COMBINED == pSrcBuffer->Layout) {
    ...
    // map combined plane
    if (RET_SUCCESS !=
        HalMapMemory_ry(hal, YCBaseAddr, YCPlaneSize, HAL_MAPMEM_READONLY,
    ...
    // map luma plane
    if (RET_SUCCESS !=
        HalMapMemory_ry(hal, YBaseAddr, YPlaneSize, HAL_MAPMEM_READONLY,
    ...
    if (RET_SUCCESS !=
      ...
      (void)HalUnMapMemory_ry(hal, pDstBuffer->Data.YCbCr.semiplanar.Y.pData);
      MEMSET(pDstBuffer, 0, sizeof(PicBufMetaData_t));
      return false;
    }
    ...
    // map Cb plane
    if (RET_SUCCESS !=
        HalMapMemory_ry(hal, CbBaseAddr, CbPlaneSize, HAL_MAPMEM_READONLY,
    ...
    // map Cr plane
    if (RET_SUCCESS !=
        HalMapMemory_ry(hal, CrBaseAddr, CrPlaneSize, HAL_MAPMEM_READONLY,
                     (void **)&(pDstBuffer->Data.YCbCr.planar.Cr.pData))) {
      (void)HalUnMapMemory_ry(hal, pDstBuffer->Data.YCbCr.planar.Cb.pData);
      (void)HalUnMapMemory_ry(hal, pDstBuffer->Data.YCbCr.planar.Y.pData);
      MEMSET(pDstBuffer, 0, sizeof(PicBufMetaData_t));
      return false;
    }
  }
...
static bool unmapRawBuffer(HalHandle_t hal, PicBufMetaData_t *pData) {
  ...
  if (RET_SUCCESS != HalUnMapMemory_ry(hal, (void *)(pData->Data.raw.pData))) {
    return false;
  }
...
static bool unmapYCbCrBuffer(HalHandle_t hal, PicBufMetaData_t *pData) {
  ...
  if (PIC_BUF_LAYOUT_COMBINED == pData->Layout) {
    if (RET_SUCCESS !=
        HalUnMapMemory_ry(hal, (void *)(pData->Data.YCbCr.combined.pData))) {
      return false;
    }
  } else if (PIC_BUF_LAYOUT_SEMIPLANAR == pData->Layout) {
    if (RET_SUCCESS !=
        HalUnMapMemory_ry(hal, (void *)(pData->Data.YCbCr.semiplanar.Y.pData))) {
      return false;
    }
    pData->Data.YCbCr.semiplanar.Y.pData = NULL;

    if (RET_SUCCESS !=
        HalUnMapMemory_ry(hal,
                       (void *)(pData->Data.YCbCr.semiplanar.CbCr.pData))) {
      return false;
    }
  } else if (PIC_BUF_LAYOUT_PLANAR == pData->Layout) {
    if (RET_SUCCESS !=
        HalUnMapMemory_ry(hal, (void *)(pData->Data.YCbCr.planar.Y.pData))) {
      return false;
    }
    pData->Data.YCbCr.planar.Y.pData = NULL;

    if (RET_SUCCESS !=
        HalUnMapMemory_ry(hal, (void *)(pData->Data.YCbCr.planar.Cb.pData))) {
      return false;
    }
    pData->Data.YCbCr.planar.Cb.pData = NULL;

    if (RET_SUCCESS !=
        HalUnMapMemory_ry(hal, (void *)(pData->Data.YCbCr.planar.Cr.pData))) {
      return false;
    }
  }

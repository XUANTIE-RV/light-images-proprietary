...
#include "types_ry.h"
#include "cam_dct_assert_ry.h"
#include "cam_device_buf_defs_common_ry.h"
...
#define PIC_BUFFER_NUM_INPUT_RY        (1)
#define PIC_BUFFER_SIZE_INPUT_RY       (8 * 1024 * 1024)    // 21MPixel@12bit => 42MByte
#define PIC_BUFFER_NUM_LARGE_IMAGE_RY   (0)
#define PIC_BUFFER_SIZE_LARGE_IMAGE_RY  (8 * 1024 * 1024)    // 21MPixel@12bit => 42MByte
#define PIC_BUFFER_NUM_SMALL_IMAGE_RY   (4)
#define PIC_BUFFER_SIZE_SMALL_IMAGE_RY  (8 * 1024 * 1024)     // 2MPixel@4:4:4 10bit  => 8MByte


#ifdef ISP_DEC
#define DEC_TILE_BUFFER_SIZE       (172032)    // (pictureSize/(tile size))*tile status bits)/8  tilesize=> 128byte tile status bits=> 4
#endif
DCT_ASSERT_STATIC(((PIC_BUFFER_NUM_INPUT_RY*PIC_BUFFER_SIZE_INPUT_RY) + (PIC_BUFFER_NUM_LARGE_IMAGE_RY*PIC_BUFFER_SIZE_LARGE_IMAGE_RY) + (PIC_BUFFER_NUM_SMALL_IMAGE_RY*PIC_BUFFER_SIZE_SMALL_IMAGE_RY)) < PIC_BUFFER_MEM_SIZE_MAX);

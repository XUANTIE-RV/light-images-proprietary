...
typedef enum CamerIcCommandId_e
{
    ...
    CAMERIC_MI_COMMAND_DMA_FINISHED     = (CAMERIC_MI_COMMAND_BASE  + 0x0001)   /**< dma completed */
} CamerIcCommandId_t;
...
typedef enum CamerIcRequestId_e
{
    ...
    CAMERIC_MI_REQUEST_GET_EMPTY_RDI_BUFFER  = (CAMERIC_MI_REQUEST_BASE + 0x0004),   /**< Request id to get an empty buffer for Rdi */
    CAMERIC_MI_REQUEST_GET_EMPTY_META_BUFFER  = (CAMERIC_MI_REQUEST_BASE + 0x0005),   /**< Request id to get an empty buffer for Meta */
} CamerIcRequestId_t;
...
#endif
    CAMERIC_MI_EVENT_FULL_RDI_BUFFER        ,                                          /**< new frame on Rdi path available */
    CAMERIC_MI_EVENT_FULL_META_BUFFER        ,                                         /**< new frame on Meta path available */
...
#endif
    CAMERIC_MI_EVENT_FLUSHED_RDI_BUFFER                                         ,     /**< delivers a flushed buffer on Rdi path */
    CAMERIC_MI_EVENT_FLUSHED_META_BUFFER                                        ,     /**< delivers a flushed buffer on Meta path */
...
#endif
    CAMERIC_MI_EVENT_DROPPED_RDI_BUFFER                                         ,     /**< informs that a frame was dropped/skipped on Rdi path */
    CAMERIC_MI_EVENT_DROPPED_META_BUFFER                                        ,     /**< informs that a frame was dropped/skipped on Meta path */
...
typedef struct pp_write_line_entry_s {
	uint32_t isp_id;
	...
	uint64_t buf_addr;
} pp_write_line_entry_t;


typedef struct CamerIcISPExtAllocPool {
	uint64_t (*alloc)(void *allocation_pool,uint32_t size, uint32_t align);
	void (*free)(void* pool,uint64_t addr,uint32_t size);
    void *pool;
}CamerIcISPExtAllocPool_t;
...
RESULT CamerIcDriverGetMiPpwBufInfo
(
    CamerIcDrvHandle_t			handle,
    pp_write_line_entry_t		*pp_wr_line_entry
);
